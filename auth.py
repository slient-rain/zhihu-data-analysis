...
def login(account=None, password=None):
    ...
    form_data = get_form_data(account, password)
    """
        result:
            {"result": True}
    ...
    if "error" in result:
        ...
        s.cookies.save()
    test_url = 'https://www.zhihu.com/people/wu-yan-de-yu/followees'
    res = s.get(test_url, headers=headers_base, verify=False)
...
def islogin():
    ...
    r = s.get(url,headers=headers_base, allow_redirects=False, verify=False)
...
def get_form_data(account, password):
...
def search_xsrf():
    ...
    r = s.get(url,headers=headers_base, verify=False)
    if int(r.status_code) != 200:
        raise NetworkError(u"XSRF码请求失败")
...
def upload_form(form):
    ...
    r = s.post(url, data=form, headers=headers, verify=False)
    if int(r.status_code) != 200:
