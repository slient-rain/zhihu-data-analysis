...
class User:
    ...
    def __init__(self, user_url, user_id=None):
        try:
            ...
            if user_url == None:
            ...
            elif ((user_url.startswith('www.zhihu.com/people', user_url.index('//') + 2) == False) and (user_url.startswith('www.zhihu.com/org', user_url.index('//') + 2) == False)):
                self.user_id = '匿名用户'
                self.followeeNum = -1
                self.followerNum = -1
                self.anserNum = -1
                self.agreeNum = -1
                self.thanksNum = -1
                logger.error("\"" + user_url + "\"" + " : it isn't a user url.")
                Logging.error("\"" + user_url + "\"" + " : it isn't a user url.")
            else:
        ...
        try:
            if hasattr(self, "user_id"):
                ...
                if self.soup == None:
                    self.parser()
                soup = self.soup
                # if soup.find("div", class_="title-section")==None or soup.find("div", class_="title-section").find("span", class_="name")==None:
                #     print '网页  ' , self.user_url , '未抓取完全，未获取到用户名'
                #     return '匿名用户'
                # else:

                user_id = soup.find("div", class_="title-section").find("span", class_="name").string
                self.user_id = user_id
                return user_id
        except:
            print '网页  ', self.user_url, '未抓取完全，未获取到用户名'
    ...
    def get_followees_num(self):
        if hasattr(self, "followees_num"):
            ...
            if self.soup == None:
                self.parser()
            soup = self.soup
            # if soup.find("div", class_="zm-profile-side-following zg-clear") ==None or  soup.find("div", class_="zm-profile-side-following zg-clear").find("a") ==None:
            #     print '网页  ' , self.user_url , '抓取不完整，未获取到关注者的人数'
            #     return -1
            # else:
            try:
                ...
                try:
                    print '网页  ', self.user_url, '未抓取完全，未获取到关注者的人数'
                    s = traceback.format_exc()
                    s += ('error url is %s' % self.user_url)
                    logger = FinalLogger.getLogger()
                    logger.error(s)
                    print '网页  ', self.user_url, '错误日志记录完毕'
                    return -1
                except:
                    s = traceback.format_exc()
                    print s
    ...
    def get_followers_num(self):
        if hasattr(self, "followees_num"):
            ...
            if self.soup==None:
                self.parser()
            soup = self.soup
            # if soup.find("div", class_="zm-profile-side-following zg-clear") ==None or soup.find("div", class_="zm-profile-side-following zg-clear").find_all("a")==None or len(soup.find("div", class_="zm-profile-side-following zg-clear").find_all("a"))<2:
            #     print '网页  ' ,self.user_url , '抓取不完整，未获取到关注人的人数'
            #     return -1
            # else:
            try:
                ...
                print '网页  ', self.user_url, '未抓取完全，未获取到关注人的人数'
                s = traceback.format_exc()
                s += ('error url is %s' % self.user_url)
                logger.error(s)
                return -1
    ...
    def get_agree_num(self):
        if hasattr(self, "followees_num"):
            ...
            if self.soup == None:
                self.parser()
            soup = self.soup
            # if soup.find("span", class_="zm-profile-header-user-agree")==None:
            #     print '网页  ', self.user_url, '抓取不完整，未获取到赞同数'
            #     return -1
            # else:
            try:
                ...
                print '网页  ', self.user_url, '未抓取完全，未获取到赞同数'
                s = traceback.format_exc()
                s += ('error url is %s' % self.user_url)
                logger = FinalLogger.getLogger()
                logger.error(s)
    ...
    def get_thanks_num(self):
        if hasattr(self, "followees_num"):
            ...
            if self.soup == None:
                self.parser()
            soup = self.soup
            # if soup.find("span", class_="zm-profile-header-user-thanks")==None:
            #     print '网页  ', self.user_url, '抓取不完整，未获取到感谢数'
            #     return -1
            # else:
            try:
                ...
                print '网页  ', self.user_url, '未抓取完全，未获取到感谢数'
                s = traceback.format_exc()
                s += ('error url is %s' % self.user_url)
                logger = FinalLogger.getLogger()
                logger.error(s)
    ...
    def get_answers_num(self):
        if hasattr(self, "followees_num"):
            ...
            soup = self.soup
            # if soup.find_all("span", class_="num")==None or len(soup.find_all("span", class_="num"))==0:
            #     print '网页  ', self.user_url, '抓取不完整，未获取到回答数'
            #     return -1
            # else:
            try:
                ...
                print '网页  ', self.user_url, '未抓取完全，未获取到回答数'
                s = traceback.format_exc()
                s += ('error url is %s' % self.user_url)
                logger = FinalLogger.getLogger()
                logger.error(s)
                    ...
                    for i in xrange((followees_num - 1) / 20 + 1):
                        if i == 0:
                            ...
                            for j in xrange(min(followees_num, 20)):
                                yield User(user_url_list[j].a["href"], user_url_list[j].a.string)
                        else:
                            ...
                            for j in xrange(min(followees_num - i * 20, 20)):
                                ...
                                yield User(user_link["href"], user_link.string)
                except:
                    print '网页  ', self.user_url, '未抓取完全，未获取到关注人列表'
                    s = traceback.format_exc()
                    s += ('error url is %s' % self.user_url)
                    logger = FinalLogger.getLogger()
                    logger.error(s)
            ...
            for i in xrange((answers_num - 1) / 20 + 1):
                if i == 0:
                    for j in xrange(min(answers_num, 20)):
                        ...
                        if soup.find_all("div", class_="zm-item-answer")[j].find("div",
                        ...
                        if is_my_answer == True:
                            count = soup.find_all("div", class_="zm-item-answer")[j].find("a",
...
class Answer:
    ...
    def get_voters(self):
        ...
        r = requests.get(request_url, params={"params": "{\"answer_id\":\"%d\"}" % int(data_aid)}, headers=headers,
                         verify=False)
        soup = BeautifulSoup(r.content, "lxml")
